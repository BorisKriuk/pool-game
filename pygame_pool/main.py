...
from utils import SCREEN_HEIGHT, SCREEN_WIDTH, FPS, BG, TABLE_IMAGE, DIA, RED, POCKET_DIA, BOTTOM_PANEL, WHITE
...
def create_ball(radius, position):
    ...
    pivot.max_force = 1000  # emulate liner friction
...
ball_images = []
for i in range(1, 17):
    ball_image = pygame.image.load(f'ball_{i}.png')
...
pygame.display.set_caption("Pool 1.0")
...
while running:
    ...
    if taking_shot and game_running:
        ...
        y_dist = -(balls[-1].body.position[1] - mouse_pos[1])  # -ve because pygame coordinates increase down the screen
    ...
    if powering_up and game_running:
        ...
        if force >= max_force or force <= 0:
            force_direction *= (-1)
        # draw power bars
        for b in range(math.ceil(force / 4000)):
    ...
    if len(balls) == 1:
        draw_text("YOU WIN", large_font, WHITE, SCREEN_WIDTH / 2 - 160, SCREEN_HEIGHT / 2 - 20)
        game_running = False

    # check if game is over
    if lives <= 0:
        draw_text("GAME OVER", large_font, WHITE, SCREEN_WIDTH / 2 - 160, SCREEN_HEIGHT / 2 - 20)
        game_running = False
